...
namespace impl {
	 ...
	 * the \c #swapPref.
	 */
	static void initSwapChain(WGPUBackendType backend, WGPUDevice device, HWND window)
	{
		switch (backend)
		{
#ifdef DAWN_ENABLE_BACKEND_D3D12
...
//******************************** Public API ********************************/
WGPUDevice createDevice(HWND handle, WGPUBackendType type)
{
	// IF type is Force32, we choose the type
	...
	if (dawn_native::Adapter adapter = impl::requestAdapter(type))
	{
		...
		impl::initSwapChain(impl::backend, impl::device, handle);
